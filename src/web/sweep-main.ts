...
async function startSweep(){ try {
  ...
  let cached:Record<string,SweepResult>={};try{cached=await lookupCache(keyed.map(item=>item.key));el('cache-status').textContent='Local cache active · data/sweep-cache.json';}catch{cacheAvailable=false;el('cache-status').textContent='Repository cache unavailable · results stay in memory and can be downloaded';}
