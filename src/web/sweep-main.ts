...
function selectResult(result: SweepResult) { selected=result; renderHeatmap(); const s=result.summary, context=runContext;
  ...
  const pidQuery=new URLSearchParams({chassis:query.get('chassis')!,board:'elle',width:String(result.lineWidthMm),radius:String(result.turnRadiusMm),duration:String(context?.durationS??n('duration')),speed:'120',motorDelay:'0.05'});
  el('cell-detail').innerHTML=`<strong>Selected:</strong> radius ${result.turnRadiusMm} mm · width ${result.lineWidthMm} mm · Kp ${result.kp}${robustnessTested(result)?' · <strong>robustness tested</strong>':''}<br><strong>Status:</strong> ${s.status} · <strong>minimum clearance:</strong> ${s.minimumClearanceMm?.toFixed(3) ?? '—'} mm · <strong>RMS error:</strong> ${s.rmsLineErrorMm?.toFixed(3) ?? '—'} mm · <strong>HF Yaw Rate RMS:</strong> ${s.hfYawRateRmsRadS.toFixed(4)} rad/s<br><a class="text-button" href="./simulate.html?${query}" target="_blank">Open these conditions in Run studio ↗</a> · <a class="text-button" href="./pid-tuning.html?${pidQuery}">Map Kp × Kd →</a> · <a class="text-button" href="./robustness.html?${robustQuery}">Test noise robustness →</a>`;
}
