...
- Signed column-centre clearance against the transformed chassis polygon.
...
- A content-addressed local sweep cache in `data/sweep-cache.json`.
...
src/web/               Page controllers, Canvas rendering, and sweep worker
tests/                 Geometry, control, simulation, sweep, and browser checks
...
- Sensor noise, channel bias, and finite optical footprints.
- Tire slip, inertia, motor dynamics, battery effects, and measured actuator limits.
- Three-way board tiles and branch-selection policy.
- Reverse recovery or line reacquisition after complete sensor loss.
- A command-line batch runner and statistical noise replicates.
...
covering all five permanent pages, cache reuse, heat-map direction, and mobile layout.
